...
/**
 * @author mog
 * @version $Id: Trial.java 1765 2007-08-04 04:14:28Z tdsoul $
 */
public class Trial extends FtpListener {
    ...
    public void actionPerformed(Event event) {
        ...
        if ("RESETDAY".equals(cmd)) {
            ...
            if (uevent.getTime() < cal.getTimeInMillis()) {
                ...
                if ((uevent.getUser().getKeyedMap().getObjectDate(UserManagement.LASTSEEN).getTime() <= cal.getTimeInMillis()) &&
                    ...
                    checkPassed(uevent.getUser(),
                        uevent.getUser().getUploadedBytesDay(), PERIOD_DAILY);
                }
            } else {
        ...
        if ("RESETWEEK".equals(cmd)) {
            if (!isInFirstPeriod(uevent.getUser(), PERIOD_WEEKLY,
                        uevent.getTime())) {
                checkPassed(uevent.getUser(),
                    uevent.getUser().getUploadedBytesWeek(), PERIOD_WEEKLY);
            }
        }

        if ("RESETMONTH".equals(cmd)) {
            if (!isInFirstPeriod(uevent.getUser(), PERIOD_MONTHLY,
                        uevent.getTime())) {
                checkPassed(uevent.getUser(),
                    uevent.getUser().getUploadedBytesMonth(), PERIOD_MONTHLY);
            }
        }
    }
